...
fh = netcdf.Dataset(directory_1500_PI+'Atmosphere/TEMP_2m_Europe_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
...
lon_global, lat_global, return_level_min	= ReadinDataGEV(directory_1500_PI+'/Atmosphere/TEMP_2m_extremes_GEV_fit_minima.nc', return_period)
...
CS      = ax.contourf(lon, lat, np.mean(temp_all, axis = 0), levels = np.arange(-30, 30.01, 2.5), extend = 'both', cmap = 'Spectral_r', transform=ccrs.PlateCarree())

divider = make_axes_locatable(ax)
ax_cb   = divider.new_horizontal(size="5%", pad=0.1, axes_class=plt.Axes)
fig.add_axes(ax_cb)

cbar    = colorbar(CS, ticks = np.arange(-30, 30.1, 10), cax=ax_cb)
...
ax.set_title('g) January temperature and 1:10-year cold extreme, AMOC on ($F_H = 0.45$ Sv)', fontsize = 11)
