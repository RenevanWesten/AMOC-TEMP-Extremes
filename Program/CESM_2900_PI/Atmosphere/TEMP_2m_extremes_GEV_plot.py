...
lon, lat, return_level_min_2900	= ReadinDataGEV(directory_2900_PI+'/Atmosphere/TEMP_2m_extremes_GEV_fit_minima.nc', return_period)
lon, lat, return_level_max_2900	= ReadinDataGEV(directory_2900_PI+'/Atmosphere/TEMP_2m_extremes_GEV_fit_maxima.nc', return_period)
...
return_level_min_plot					= return_level_min_2900 - return_level_min_1500
...
return_level_max_plot					= return_level_max_2900 - return_level_max_1500
...
for lat_i in range(0, len(lat), 3):
    for lon_i in range(0, len(lon), 3):
        ...
        if return_level_max_2900[lat_i, lon_i] >= 40:
            #Non-significant difference
            ax.scatter(lon[lon_i], lat[lat_i], marker = 'o', edgecolor = 'k' , s = 6, facecolors='none', transform=ccrs.PlateCarree())

ax.set_title('h) 1:10-year warm extreme, AMOC off ($F_H = 0.45$ Sv)')
