...
fh = netcdf.Dataset(directory_0600_PI+'Atmosphere/Meridional_SHF_LHF_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
...
ax.plot(lat, SHF_global, '-', color = 'r', linewidth = 2.0)
ax.plot(lat, SHF_atl, '--', color = 'firebrick', linewidth = 2.0)

ax.plot(lat, LHF_global, '-', color = 'b', linewidth = 2.0)
ax.plot(lat, LHF_atl, '--', color = 'royalblue', linewidth = 2.0)
...
ax.set_title('a) MHT (January), AMOC on ($F_H = 0.18$ Sv)')
...
ax.plot(lat, SHF_eddy_global, '-', color = 'r', linewidth = 2.0)
ax.plot(lat, SHF_eddy_atl, '--', color = 'firebrick', linewidth = 2.0)

ax.plot(lat, LHF_eddy_global, '-', color = 'b', linewidth = 2.0)
ax.plot(lat, LHF_eddy_atl, '--', color = 'royalblue', linewidth = 2.0)
...
ax.set_title('b) Eddy MHT (January), AMOC on ($F_H = 0.18$ Sv)')
...
ax.plot(SHF_eddy_atl, lat, '--', color = 'firebrick', linewidth = 2.0)
ax.plot(LHF_eddy_atl, lat, '--', color = 'royalblue', linewidth = 2.0)

ax.set_xlabel('MHT (PW)', fontsize = 14)
ax.set_ylim(20, 80)
ax.set_xlim(-1, 1)
ax.grid()
...
legend_labels = [l.get_label() for l in graphs]
legend_1      = ax.legend(graphs, legend_labels, loc = 'center left', ncol=1, numpoints = 1, framealpha = 1.0).set_zorder(20)

fig.subplots_adjust(top=0.915,bottom=0.132,left=0.171,right=0.926,hspace=0.2,wspace=0.2)

fig.suptitle('b) Eddy MHT, AMOC on ($F_H = 0.18$ Sv)', fontsize = 13)

#-----------------------------------------------------------------------------------------	
#-----------------------------------------------------------------------------------------	
#-----------------------------------------------------------------------------------------	
	
fh = netcdf.Dataset(directory_0600_PI+'Atmosphere/SHF_LHF_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
...
SHF_plot        = np.mean(SHF, axis = 0)
SHF_eddy_plot   = np.mean(SHF_eddy, axis = 0)
LHF_plot        = np.mean(LHF, axis = 0)
LHF_eddy_plot   = np.mean(LHF_eddy, axis = 0)
...
CS      = ax.contourf(lon, lat, SHF_plot, levels = np.arange(-5, 5.01, 0.5), extend = 'both', cmap = 'Spectral_r', transform=ccrs.PlateCarree())

divider = make_axes_locatable(ax)
ax_cb   = divider.new_horizontal(size="5%", pad=0.1, axes_class=plt.Axes)
fig.add_axes(ax_cb)

cbar    = colorbar(CS, ticks = np.arange(-5, 5.01, 1), cax=ax_cb)
cbar.set_label('Meridional heat transport (PW)')
...
ax.set_title('a) MHT$_{\mathrm{S}}$ (January), AMOC on ($F_H = 0.18$ Sv)')

#-----------------------------------------------------------------------------------------

fig, ax = plt.subplots(subplot_kw={'projection': ccrs.PlateCarree()})

CS      = ax.contourf(lon, lat, SHF_eddy_plot, levels = np.arange(-0.1, 0.101, 0.01), extend = 'both', cmap = 'Spectral_r', transform=ccrs.PlateCarree())

divider = make_axes_locatable(ax)
ax_cb   = divider.new_horizontal(size="5%", pad=0.1, axes_class=plt.Axes)
fig.add_axes(ax_cb)

cbar    = colorbar(CS, ticks = np.arange(-0.1, 0.101, 0.05), cax=ax_cb)
cbar.set_label('Meridional heat transport (PW)')
...
ax.set_title('b) MHT$_{\mathrm{S,eddy}}$ (January), AMOC on ($F_H = 0.18$ Sv)')
...
CS      = ax.contourf(lon, lat, LHF_plot, levels = np.arange(-5, 5.01, 0.5), extend = 'both', cmap = 'Spectral_r', transform=ccrs.PlateCarree())
...
ax.set_title('MHT by LHF (January), AMOC on ($F_H = 0.18$ Sv)')
...
CS      = ax.contourf(lon, lat, LHF_eddy_plot, levels = np.arange(-0.1, 0.101, 0.01), extend = 'both', cmap = 'Spectral_r', transform=ccrs.PlateCarree())
...
ax.set_title('Eddy MHT by LHF (January), AMOC on ($F_H = 0.18$ Sv)')
