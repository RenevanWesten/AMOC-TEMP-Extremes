...
fh 		    = netcdf.Dataset(directory_0600_PI+'Atmosphere/UVEL_TEMP_Atlantic_sector_month_'+str(month_start)+'-'+str(month_end)+'.nc', 'r')
...
CS	= ax.contourf(lat, pres, np.mean(u_vel, axis = 0), levels = np.arange(-50, 50.1, 5), extend = 'both', cmap = 'RdBu_r')
cbar	= colorbar(CS, ticks = np.arange(-50, 50.1, 10))
...
ax.set_title('b) Zonal velocity, AMOC on ($F_H = 0.18$ Sv)')
...
CS	= ax.contourf(lat, pres, np.mean(temp, axis = 0), levels = np.arange(250, 500.1, 10), extend = 'both', cmap = 'Spectral_r')
cbar	= colorbar(CS, ticks = np.arange(250, 500.1, 50))
...
fig.suptitle('a) Potential temperature, AMOC on ($F_H = 0.18$ Sv)')
...
fig.suptitle('b) Thermal wind balance, AMOC on ($F_H = 0.18$ Sv)')
